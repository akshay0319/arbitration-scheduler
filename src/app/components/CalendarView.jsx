...
export default function CalendarView({
  ...
  const renderHeader = (label) => (
    <div className="flex justify-between items-center mb-2">
      <h2 className="text-xl font-bold" style={{ color: "#005186" }}>
        {label}
      </h2>
      ...
      <div>
        ...
        <div className="grid grid-cols-7 text-center font-medium pb-2">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
            <div key={day} className="text-sm" style={{ color: "#005186" }}>
              {day}
            </div>
          ))}
        </div>
        {weeks.map((week, weekIndex) => (
          <div key={weekIndex} className="grid grid-cols-7 ">
            {week.map((date, i) => (
              <div
                key={i}
                className="pt-2 pb-2 border-r border-b border-[#ced4da] h-24 text-center overflow-hidden cursor-pointer hover:bg-green-50"
                onClick={() =>
                  date && onSlotClick(new Date(`${date.toDateString()} 09:00`))
                } // ✅ Ensures full datetime for modal
              >
                  ...
                  <div className="mt-1 h-[60px] overflow-y-auto space-y-1">
                    {sessions
                      ...
                      .map((s, i) => (
                        ...
                        >
                          {s.startTime} - {s.endTime} • {s.caseNumber?.slice(0, 6)}
 ...
 if (viewType === "week") {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-[100px_repeat(7,_1fr)] gap-1">
        {/* Time Column */}
        <div className="w-[100px] border-r border-[#ced4da] bg-gray-50 text-right pr-2 pt-[28px]">
          {hours.map((hour) => (
            <div
              key={hour}
              className="h-[40px] text-xs text-[#495057] border-b border-[#ced4da] flex items-center justify-end pr-2"
            >
              {formatTime(hour)}
            </div>
          ))}
        ...
        {days.map((day) => (
          <div key={day} className="border border-[#ced4da]">
            <div
              className="bg-gray-100 text-center font-medium h-[28px] flex items-center justify-center"
              style={{ color: "#005186" }}
            >
            ...
            {hours.map((hour) => {
              ...
              return (
                <div
                  key={hour}
                  className={`h-[40px] text-sm border-t border-[#ced4da] cursor-pointer px-2 py-1 ${
                    occupied
                  ...
                  onClick={() =>
                    occupied
                      ? onEdit(session.id)
                      : onSlotClick(new Date(`${day.toDateString()} ${hour}`))
                  }
                >
                  {occupied ? `${session.startTime} - ${session.endTime}` : ""}
                </div>
              );
            })}
          </div>
              ...
              <div
                ...
                onClick={() =>
                  occupied
                    ? onEdit(session.id)
                    : onSlotClick(
                        new Date(`${currentDate.toDateString()} ${hour}`)
                      )
                }
              >
